...
SELECT * FROM (
    ...
    DENSE_RANK() OVER (
        PARTITION BY customer_id
        ORDER BY market_date DESC   -- reverse the order
    ...
    TRIM(
        CASE 
            WHEN INSTR(product_name, '-') > 0 THEN
                SUBSTR(product_name, INSTR(product_name, '-') + 1) ELSE NULL
        END
    ) AS description
FROM product;
...
SELECT market_date, total_sales, sales_rank_desc AS type
FROM ranked_sales
WHERE sales_rank_desc = 1
UNION ALL  
SELECT market_date, total_sales, sales_rank_asc AS type
FROM ranked_sales
...
SELECT 
    v.vendor_name,
    p.product_name,
    SUM(vi.original_price * 5) AS potential_revenue
